...
def parse_gaussian(fd):
    ...
    while 1:
        ...
        if line.startswith(charge_marker):
            ...
            new_freqs = (float(x) for x in line[len(frequencies_marker):].strip().split())
            out.setdefault("vibrational_temperatures", []).extend(new_freqs)
        elif line.startswith(rotational_temperatures_marker):
            lst = [float(x) for x in line[len(rotational_temperatures_marker):].strip().split()]
            lst.reverse()
            ...
            out["zero_point_energy"] = ureg.Quantity(float(line[len(zpc_marker):].strip().split()[0]), "hartree")
    ...
    if "vibrational_temperatures" in out:
        out["vibrational_temperatures"] = ureg.Quantity(array(out["vibrational_temperatures"]), "reciprocal_centimeter")
    return out
